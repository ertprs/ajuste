...
class ImportacaoManualFortpagamento extends Component {
  constructor (props) {
    super(props)
    this.state = {
      files: [],
  ...
  sendFilesServer = async (e) => {
    Swal.fire({
      title: 'Continuar?',
      text: 'Deseja realmente importar um novo arquivo?',
      type: 'warning',
      ...
      if (result.value) {
        ...
        try {
          const resultado = await Api.post(`fortpagamento/manual_import/`, data, { headers: {
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Repasse Lojista - Importação Manual'} />
...
export default withStyles(styles)(ImportacaoManualFortpagamento)
