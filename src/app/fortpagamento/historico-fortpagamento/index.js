...
class HistoricoRepasseLojista extends Component {
  constructor (props) {
    super(props)
  ...
  gerarRelatorio = () => {
    exportCSVFile(this.state.dataTable, `relatorio_repasse_lojista_sintetico`, [])
    this.setState({
      snackbar: {
        open: true,
        variant: 'success',
  ...
  requestService = async (dates = { ...this.state.filter }) => {
    try {
      ...
      const result = await Api.get(`fortpagamento/historic/?data_inicial=${dates.dataInicial}&data_final=${dates.dataFinal}`)
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Repasse Lojista - Fortpagamento'} />
...
export default withStyles(styles)(HistoricoRepasseLojista)
