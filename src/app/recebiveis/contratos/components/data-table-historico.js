...
class DataTableHistorico extends React.Component {
  ...
  gerarRelatorio = async () => {
    ...
    let listaDatas = data.filter((value) => selected.includes(value.dt_registro))

    try {
      const resultado = await Api.post(`recebiveis/relatorio_analitico_contratos/`, listaDatas)
      ...
      exportCSVFile(resultado.data, `relatorio_contrato`, [])
      this.setState({
        loadRelatorio: false,
        snackbar: {
          open: true,
  ...
  handleSelectAllClick = event => {
    if (event.target.checked) {
      this.setState(state => ({ selected: state.data.map((value) => value.dt_registro) }))
      return
    }
            ...
            <Table className={classes.table} aria-labelledby='tableTitle'>
              ...
              <TableBody>
                { stableSort(data, getSorting(order, orderBy))
                  ...
                  .map((item, index) => {
                    const isSelected = this.isSelected(item.dt_registro)
                    return (
                      <TableRow
                      ...
                      >
                        <TableCell padding='checkbox'>
                          <Checkbox
                            onClick={event => (this.handleClick(event, item.dt_registro))}
                            checked={isSelected}
                          />
                        </TableCell>
                        ...
                        <TableCell className={classes.cellRowStyle} align='left'>
                          {this.prettyNumber(item.valor_ct).toLocaleString()}
                        </TableCell>
                        <TableCell className={classes.cellRowStyle} align='left'>
                          {this.prettyNumber(item.valor_executado).toLocaleString()}
                        </TableCell>
                        <TableCell className={classes.cellRowStyle} align='left'>
                          {this.prettyNumber(item.saldo).toLocaleString()}
                        </TableCell>
