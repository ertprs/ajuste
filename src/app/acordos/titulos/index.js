...
class TitulosAcordos extends Component {
  componentWillMount () {
    this.requestService()
  }
  ...
  requestService = async () => {
    try {
      const resultado = await Api.get(`acompanhamentos-acordos/titulos/?data_emissao=${this.props.location.state.dt_emissao}`)
  ...
  gerarRelatorio = () => {
    exportCSVFile(this.state.data, `relatorio_titulos_acordos`, [])
    this.setState({
      snackbar: {
        open: true,
        variant: 'success',
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Titulos - Acordos'} />
...
export default withStyles(styles)(TitulosAcordos)
