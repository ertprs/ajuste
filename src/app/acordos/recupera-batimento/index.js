...
class RecuperaBatimento extends Component {
  componentWillMount () {
    this.requestService()
  }
  ...
  requestService = async () => {
    try {
      const resultado = await Api.get(`recupera/difference/?dt_payment=${this.props.location.state.dt_payment}`)
  ...
  gerarRelatorio = () => {
    exportCSVFile(this.state.data, `relatorio_batimento_recupera`, [])
    this.setState({
      snackbar: {
        open: true,
        variant: 'success',
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Recupera - Batimento'} />
...
export default withStyles(styles)(RecuperaBatimento)
