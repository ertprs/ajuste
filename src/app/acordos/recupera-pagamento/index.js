...
class RecuperaPagamento extends Component {
  componentWillMount () {
    this.requestService()
  }
  ...
  requestService = async () => {
    try {
      const resultado = await Api.get(`recupera/paid/?dt_payment=${this.props.location.state.dt_payment}`)
  ...
  gerarRelatorio = () => {
    exportCSVFile(this.state.data, `relatorio_pagamento_recupera`, [])
    this.setState({
      snackbar: {
        open: true,
        variant: 'success',
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Recupera - Pagamentos'} />
...
export default withStyles(styles)(RecuperaPagamento)
