...
import DataTableHistorico from './components/data-table-retorno'
...
class RegistradosComplementarAcordos extends Component {
  componentWillMount () {
    this.requestService()
  }
  ...
  requestService = async () => {
    try {
      const resultado = await Api.get(`acompanhamentos-acordos/dados_retorno/?data_emissao=${this.props.location.state.dt_emissao}`)
  ...
  gerarRelatorio = () => {
    exportCSVFile(this.state.data, `relatorio_registrados_complementar_acordos`, [])
    this.setState({
      snackbar: {
        open: true,
        variant: 'success',
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Acordos - Complementar Registrados'} />
...
export default withStyles(styles)(RegistradosComplementarAcordos)
