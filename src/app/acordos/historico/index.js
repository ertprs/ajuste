...
class AcompanhamentoAcordos extends Component {
  componentWillMount () {
  ...
  requestService = async (filter = { dataInicial: this.state.filter.dataInicial, dataFinal: this.state.filter.dataFinal }) => {
    try {
      const resultado = await Api.get(`acompanhamentos-acordos/historico/?data_inicial=${filter.dataInicial}&data_final=${filter.dataFinal}`)
  ...
  gerarRelatorio = () => {
    exportCSVFile(this.state.table ? this.state.dataMonth : this.state.data, `relatorio_historico_acordos`, [])
    this.setState({
      snackbar: {
        open: true,
        variant: 'success',
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Acompanhamento Acordos'} />
...
export default withStyles(styles)(AcompanhamentoAcordos)
