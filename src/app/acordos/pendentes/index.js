...
class Pendentes extends Component {
  ...
  requestService = async () => {
    try {
      const resultado = await Api.get(`acompanhamentos-acordos/pendentes/?data_emissao=${this.props.location.state.dt_emissao}`)
  ...
  gerarRelatorio = () => {
    exportCSVFile(this.state.data, `relatorio_erros_acordos`, [])
    this.setState({
      snackbar: {
        open: true,
        variant: 'success',
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Acordos - Erros operacionais'} />
...
export default withStyles(styles)(Pendentes)
