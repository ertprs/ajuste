...
import DataTableHistorico from './components/data-table-batimento'
...
class BatimentoRetornoComplementarAcordos extends Component {
  componentWillMount () {
    this.requestService()
  }
  ...
  requestService = async () => {
    try {
      const resultado = await Api.get(`acompanhamentos-acordos/batimento_retorno_complementar/?data_emissao=${this.props.location.state.dt_emissao}`)
  ...
  gerarRelatorio = () => {
    exportCSVFile(this.state.data, `relatorio_batimento_acordos_ret_comp`, [])
    this.setState({
      snackbar: {
        open: true,
        variant: 'success',
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Batimento RET X COMP - Acordos'} />
...
export default withStyles(styles)(BatimentoRetornoComplementarAcordos)
