...
import DataTableHistorico from './components/data-table-complementar'
...
class ComplementarAcordos extends Component {
  componentWillMount () {
    this.requestService()
  }
  ...
  requestService = async () => {
    try {
      const resultado = await Api.get(`acompanhamentos-acordos/dados_complementar/?data_emissao=${this.props.location.state.dt_emissao}`)
  ...
  gerarRelatorio = () => {
    exportCSVFile(this.state.data, `relatorio_arquivo_complementar`, [])
    this.setState({
      snackbar: {
        open: true,
        variant: 'success',
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Complementar - Acordos'} />
...
export default withStyles(styles)(ComplementarAcordos)
