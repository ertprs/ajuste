...
class RecuperaRegistrados extends Component {
  componentWillMount () {
    this.requestService()
  }
  ...
  requestService = async () => {
    try {
      const resultado = await Api.get(`recupera/registered/?dt_payment=${this.props.location.state.dt_payment}`)
  ...
  gerarRelatorio = () => {
    exportCSVFile(this.state.data, `relatorio_registrados_recupera`, [])
    this.setState({
      snackbar: {
        open: true,
        variant: 'success',
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Recupera - Registrados'} />
              ...
              <Grid container style={{ marginTop: '10px' }} direction='column' justify='space-around' alignItems='flex-end'>
                <IconButton
                  onClick={() => this.props.history.push(
                    '/app/acordos/historico-parcelas-recupera',
                    {
...
export default withStyles(styles)(RecuperaRegistrados)
