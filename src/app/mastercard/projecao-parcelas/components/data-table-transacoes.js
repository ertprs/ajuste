...
class DataTableHistorico extends React.Component {
  ...
  render () {
    const { classes, innerLoading, dataHead, saldoAcumulado, filter } = this.props
    const { data, order, orderBy, selected, rowsPerPage, page } = this.state
    const liquidacaoT140 = data.map(row => parseFloat(row.liquidacao_t140)).reduce((acc, row) => row + acc)
    const comissaoT140 = data.map(row => parseFloat(row.comissao_t140)).reduce((acc, row) => row + acc)

    const totalBaseUnica = data.map(row => parseFloat(row.projecao_base_unica)).reduce((acc, row) => row + acc)
    const totalDivergencia = data.map(row => parseFloat(row.divergencia)).reduce((acc, row) => row + acc)
    return (
      <MuiThemeProvider theme={muiDatatableTheme}>
        <Grid container direction='row' justify='flex-end'>
          <Typography style={textAcumuladoCss} >Saldo Acumulado</Typography>
          ...
          <Table className={classes.table} aria-labelledby='tableTitle'>
            ...
            <TableBody>
                    ...
                    return (
                      ...
                      >
                        <TableCell className={classes.cellRowStyle} align='left'>
                          {item.dt_arquivo_t140}
                        </TableCell>
                        <TableCell className={classes.cellRowStyle} align='left'>
                          {item.dt_agenda}
                        </TableCell>
                        <TableCell className={classes.cellRowStyle} align='left'>
                          {this.prettyNumber(item.liquidacao_t140)}
                        </TableCell>
                        <TableCell className={classes.cellRowStyle} align='left'>
                          {this.prettyNumber(item.comissao_t140)}
                        </TableCell>
                        <TableCell className={classes.cellRowStyle} align='left'>
                          ...
                          <IconButton
                            ...
                            onClick={() =>
                              this.props.history.push(
                                '/app/mastercard/batimento-parcelas-master-agrupadas',
                                {
                                  dt_agenda: this.FormatdateApi(item.dt_agenda),
                                  filter: filter
                                }
                              )
                            }
              ...
              <TableRow>
                ...
                <TableCell className={classes.cellRowStyle} align='left' />
                <TableCell className={classes.cellRowStyle} align='left'>
                  {this.prettyNumber(liquidacaoT140)}
                </TableCell>
                <TableCell className={classes.cellRowStyle} align='left'>
                  {this.prettyNumber(comissaoT140)}
                </TableCell>
                <TableCell className={classes.cellRowStyle} align='left'>
                  {this.prettyNumber(totalBaseUnica)}
                </TableCell>
                <TableCell className={classes.cellRowStyle} align='left'>
                  {this.prettyNumber(totalDivergencia)}
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
