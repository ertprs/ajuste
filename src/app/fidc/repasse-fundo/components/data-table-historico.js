...
class DataTableHistorico extends React.Component {
  ...
  gerarRelatorio = async () => {
    ...
    try {
      const resultado = await Api.post(`fidc/relatorio_retorno/`, listaDatas)
      ...
      exportCSVFile(resultado.data, `relatorio_fidc_retorno`, [])
      this.setState({
        loadRelatorio: false,
        snackbar: {
          open: true,
  ...
  gerarRelatorioSyspag = async () => {
    ...
    try {
      const resultado = await Api.post(`fidc/relatorio_retorno_syspag/`, listaDatas)
      ...
      exportCSVFile(resultado.data, `relatorio_repasse_syspag`, [])
      this.setState({
        loadRelatorio: false,
        snackbar: {
          open: true,
  ...
  render () {
    ...
    const saldoAcumulado = data.length ? data.map(row => parseFloat(row.vl_liquido_ant)).reduce((acc, row) => row + acc) : 0

    return (
      <MuiThemeProvider theme={muiDatatableTheme}>
        <div className={classes.filterContainer}>
          <FilterData
            relatorioFuturoLoad={this.props.relatorioFuturoLoad}
            ...
            gerarRelatorioFuturo={this.props.gerarRelatorioFuturo}
          />
        </div>
        <Snackbar
                  ...
                  .map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className={classes.cellRowStyle} align='left'>
                          {parseInt(item.qtd_lojistas).toLocaleString()}
                        </TableCell>
                        <TableCell className={classes.cellRowStyle} align='left'>
                          {this.prettyNumber(item.vl_liquido_ant).toLocaleString()}
                        </TableCell>
                        <TableCell className={classes.cellRowStyle} align='left'>
                          {this.prettyNumber(item.vl_arquivo).toLocaleString()}
                        </TableCell>
                        <TableCell className={classes.cellRowStyle} align='left'>
                          {this.prettyNumber(item.verificacao).toLocaleString()}
                        </TableCell>
