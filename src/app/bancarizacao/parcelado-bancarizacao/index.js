...
class ParceladoBancarizacao extends Component {
  constructor (props) {
    super(props)
  ...
  requestService = async (dates = { ...this.state.filter }) => {
    try {
      ...
      const result = await Api.get(`bancarizacao/operacoes_parcelado/?data_inicial=${dates.dataInicial}&data_final=${dates.dataFinal}`)
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Parcelado Bancarização'} />
...
export default withStyles(styles)(ParceladoBancarizacao)
