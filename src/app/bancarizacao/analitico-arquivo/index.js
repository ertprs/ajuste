...
class AnaliticoArquivo extends Component {
  constructor (props) {
    super(props)
  ...
  gerarRelatorio = () => {
    exportCSVFile(this.state.dataTable, `relatorio_analitico_bancarizacao`, [])
    this.setState({
      snackbar: {
        open: true,
        variant: 'success',
  ...
  requestService = async () => {
    try {
      const { dataArquivo, dataOperacao, tipo } = this.props.location.state

      const result = await Api.get(`bancarizacao/analitico_parcelado_arquivo/?data_arquivo=${dataArquivo}&data_operacao=${dataOperacao}&tipo=${tipo}`)
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Arquivo Analítico'} />
...
export default withStyles(styles)(AnaliticoArquivo)
