...
class ArquivoRejeicao extends Component {
  constructor (props) {
    super(props)
  ...
  gerarRelatorio = () => {
    exportCSVFile(this.state.dataTable, `relatorio_arquivo_rejeicao`, [])
    this.setState({
      snackbar: {
        open: true,
        variant: 'success',
  ...
  requestService = async () => {
    try {
      ...
      const result = await Api.get(`bancarizacao/arquivo_rejeicao/?nome_arquivo=${nomeArquivo}`)
  ...
  render () {
    ...
    return (
      <Fragment>
        ...
        />
        <TitlePage text={'Arquivo Rejeicao'} />
...
export default withStyles(styles)(ArquivoRejeicao)
