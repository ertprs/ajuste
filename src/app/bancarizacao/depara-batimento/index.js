...
class BatimentoDepara extends Component {
  constructor (props) {
    super(props)
  ...
  gerarRelatorio = () => {
    exportCSVFile(this.state.dataTable, `relatorio_batimento_ccbs`, [])
    this.setState({
      snackbar: {
        open: true,
        variant: 'success',
  ...
  requestService = async () => {
    try {
      ...
      const result = await Api.get(`bancarizacao/batimento_depara/?nome_arquivo=${nomeArquivo}`)
...
export default withStyles(styles)(BatimentoDepara)
